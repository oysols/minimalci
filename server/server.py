...
def build_all_branches() -> None:
    ...
    for branch, sha in get_new_branches():
        ...
        # Clean up
        if state.secrets:
            executors.safe_del_tmp_file(state.secrets.path)
        if state.source:
            executors.safe_del_tmp_file(state.source.path)
        shutil.rmtree(workdir)
